...
const isDifferent = (s1: StudentRecord, s2: StudentRecord) => {
  if (!s1 || !s2) return false;
  if (s1.name !== s2.name) return true;
  if ((s1.rollNo || "") !== (s2.rollNo || "")) return true;
  if (s1.grade !== s2.grade) return true;
  if ((s1.phase || "") !== (s2.phase || "")) return true;
  if ((s1.batch || "") !== (s2.batch || "")) return true;
  
  if (s1.scores.participation !== s2.scores.participation) return true;
  if (s1.scores.homework !== s2.scores.homework) return true;
  if (s1.scores.mcq !== s2.scores.mcq) return true;
  if (s1.scores.project !== s2.scores.project) return true;
  if (s1.scores.lab !== s2.scores.lab) return true;
  
  if ((s1.remarks?.participation || "") !== (s2.remarks?.participation || "")) return true;
  if ((s1.remarks?.homework || "") !== (s2.remarks?.homework || "")) return true;
  if ((s1.remarks?.mcq || "") !== (s2.remarks?.mcq || "")) return true;
  if ((s1.remarks?.project || "") !== (s2.remarks?.project || "")) return true;
  if ((s1.remarks?.lab || "") !== (s2.remarks?.lab || "")) return true;

  if ((s1.strengths || "") !== (s2.strengths || "")) return true;
  if ((s1.areasOfImprovement || "") !== (s2.areasOfImprovement || "")) return true;
...
export default function App() {
  ...
  const [tempCloudPayload, setTempCloudPayload] = useState<any>(null);
  ...
  const applyPulledData = (payload: any, silent = false) => {
    ...
    if (payload.schoolName) {
      ...
      localStorage.setItem("edugrade_schoolName", payload.schoolName);
    }
    if (payload.schoolMotto) {
      setSchoolMotto(payload.schoolMotto);
      localStorage.setItem("edugrade_schoolMotto", payload.schoolMotto);
    }
    if (payload.schoolLogo) {
      setSchoolLogo(payload.schoolLogo);
      localStorage.setItem("school_logo", payload.schoolLogo);
    } else {
      setSchoolLogo(null);
      localStorage.removeItem("school_logo");
    }
    if (payload.allowedClasses) {
      setAllowedClasses(payload.allowedClasses);
      localStorage.setItem("edugrade_allowed_classes", JSON.stringify(payload.allowedClasses));
    }
    if (payload.allowedBatches) {
      setAllowedBatches(payload.allowedBatches);
      localStorage.setItem("edugrade_allowed_batches", JSON.stringify(payload.allowedBatches));
    }
    ...
    setHasUnsaved(false);
    if (!silent) {
      triggerStatus("🔄 Successfully pulled latest evaluations from Cloud Database!");
    }
  };

  const pullDataFromServer = async (silent = false): Promise<"success" | "uninitialized" | "conflict" | "error"> => {
    setIsSyncing(true);
    try {
      ...
      if (data.initialized) {
        ...
        // Compare each local record with cloud record to find conflicts
        localStudents.forEach(localRec => {
          const cloudRec = fetchedStudents.find((c: StudentRecord) => c.id === localRec.id);
          if (cloudRec) {
            if (isDifferent(localRec, cloudRec)) {
              conflictsFound.push({
                id: localRec.id,
                name: localRec.name,
                local: localRec,
                cloud: cloudRec,
                choice: "cloud" // default choice is cloud
              });
            }
          }
        });
        ...
        if (conflictsFound.length > 0) {
          ...
          setTempCloudPayload(data);
          setConflictModalOpen(true);
          if (!silent) {
            triggerStatus(`⚠️ Found ${conflictsFound.length} conflicting student records! Please select which version to keep.`);
          }
          return "conflict";
        }

        // Apply pulled data directly if there are no conflicts
        applyPulledData(data, silent);
        return "success";
      } else {
        if (!silent) {
          triggerStatus("⚠️ Cloud database is empty or uninitialized.");
        }
      ...
      setIsOffline(true);
      if (!silent) {
        triggerStatus("⚠️ Pull failed: Could not connect to server database.");
      }
      return "error";
    } finally {
      setIsSyncing(false);
  ...
  const handleResolveConflicts = async () => {
    if (!tempCloudPayload) return;
    
    setIsSyncing(true);
    try {
      const fetchedStudents = tempCloudPayload.students || [];
      const resolvedStudents: StudentRecord[] = [];
      const localStudents = students;

      const allIds = Array.from(new Set([
        ...localStudents.map(s => s.id),
        ...fetchedStudents.map((s: StudentRecord) => s.id)
      ]));
      ...
      allIds.forEach(id => {
        ...
        if (conflict) {
          if (conflict.choice === "local") {
            resolvedStudents.push(conflict.local);
          } else {
            resolvedStudents.push(conflict.cloud);
          }
        } else {
          const localRec = localStudents.find(s => s.id === id);
          const cloudRec = fetchedStudents.find((s: StudentRecord) => s.id === id);

          if (localRec && cloudRec) {
            resolvedStudents.push(localRec);
          } else if (localRec) {
            resolvedStudents.push(localRec);
          } else if (cloudRec) {
            resolvedStudents.push(cloudRec);
          }
        }
      });
      ...
      localStorage.setItem("edugrade_students", JSON.stringify(resolvedStudents));

      if (tempCloudPayload.schoolName) {
        setSchoolName(tempCloudPayload.schoolName);
        localStorage.setItem("edugrade_schoolName", tempCloudPayload.schoolName);
      }
      if (tempCloudPayload.schoolMotto) {
        setSchoolMotto(tempCloudPayload.schoolMotto);
        localStorage.setItem("edugrade_schoolMotto", tempCloudPayload.schoolMotto);
      }
      if (tempCloudPayload.schoolLogo) {
        setSchoolLogo(tempCloudPayload.schoolLogo);
        localStorage.setItem("school_logo", tempCloudPayload.schoolLogo);
      }
      if (tempCloudPayload.allowedClasses) {
        setAllowedClasses(tempCloudPayload.allowedClasses);
        localStorage.setItem("edugrade_allowed_classes", JSON.stringify(tempCloudPayload.allowedClasses));
      }
      if (tempCloudPayload.allowedBatches) {
        setAllowedBatches(tempCloudPayload.allowedBatches);
        localStorage.setItem("edugrade_allowed_batches", JSON.stringify(tempCloudPayload.allowedBatches));
      }
      ...
      setHasUnsaved(false);
      triggerStatus("🎉 Successfully resolved all conflicts and synchronized local & cloud databases!");
      setConflictModalOpen(false);
      setConflicts([]);
      setTempCloudPayload(null);
    } catch (err) {
      console.error("Cloud save after resolution error:", err);
      triggerStatus("⚠️ Local data merged, but failed to upload to cloud database.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const pushDataToServer = async (silent = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setHasUnsaved(false);
        if (!silent) {
          triggerStatus("☁️ Successfully pushed current device data to Cloud Database!");
        }
        return true;
      } else {
        if (!silent) {
          triggerStatus("⚠️ Cloud save rejected by server.");
        }
        return false;
      }
    } catch (err) {
      ...
      setIsOffline(true);
      if (!silent) {
        triggerStatus("⚠️ Push failed: Could not upload to server database.");
      }
      return false;
    } finally {
      setIsSyncing(false);
  ...
  // Initial load: Synchronize with server-side cloud database
  useEffect(() => {
    async function loadServerData() {
      try {
        const result = await pullDataFromServer(true);
        if (result === "success") {
          setIsOffline(false);
          ...
          if (!initRes.ok) {
            throw new Error(`Server returned HTTP status ${initRes.status}`);
          }
          ...
          setLastSyncTime(timeStr);
          localStorage.setItem("edugrade_last_sync_time", timeStr);
          triggerStatus("☁️ Successfully registered and initialized secure Cloud database!");
        } else if (result === "conflict") {
          setIsOffline(false);
          triggerStatus("⚠️ Sync Conflict: Local edits on this device conflict with Cloud data.");
        }
      } catch (err) {
        console.error("Cloud synchronization error:", err);
        setIsOffline(true);
        triggerStatus("⚠️ Network: Running in offline local backup mode.");
      } finally {
  ...
  // Persistence (Auto-saving enabled to ensure local data is always safe)
  useEffect(() => {
  ...
  // Synchronize state changes to server-side database (Debounced and only if has unsaved changes!)
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        ...
        console.error("Failed to automatically synchronize with server database:", err);
        setIsOffline(true);
      } finally {
        setIsSyncing(false);
      }
  ...
  const handleManualSaveAll = async (customMsg?: string) => {
    try {
      localStorage.setItem("edugrade_students", JSON.stringify(students));
      localStorage.setItem("edugrade_schoolName", schoolName);
      localStorage.setItem("edugrade_schoolMotto", schoolMotto);
      ...
      const success = await pushDataToServer(true);
      if (success) {
        triggerStatus(customMsg || "🎉 Saved all progress successfully to Cloud & local storage!");
      } else {
        triggerStatus("⚠️ Saved locally, but failed to sync to Cloud. Check your network.");
      }
    } catch (e) {
      console.error("Local storage save error:", e);
      triggerStatus("⚠️ Failed to save to local storage.");
    }
  };
  ...
  const handleSpreadsheetUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const nameIdx = headers.findIndex(h => h.includes("student name") || h === "name" || h.includes("student_name"));
        const gradeIdx = headers.findIndex(h => h.includes("grade") || h === "class" || h.includes("class/section"));
        ...
        for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
          ...
          const studentName = nameIdx !== -1 && row[nameIdx] !== undefined && row[nameIdx] !== null ? String(row[nameIdx]).trim() : "";
          
          if (!studentId && !studentName) continue;

          // Find if student exists by ID, or falls back to name match to avoid duplicates
          let studentIndex = -1;
          if (studentId) {
            studentIndex = updatedStudents.findIndex(s => s.id === studentId);
          }
          if (studentIndex === -1 && studentName) {
            studentIndex = updatedStudents.findIndex(s => s.name.toLowerCase() === studentName.toLowerCase());
          }
          ...
          if (studentIndex !== -1) {
            ...
            if (rollIdx !== -1 && row[rollIdx] !== undefined && row[rollIdx] !== null && String(row[rollIdx]).trim() !== "") {
              s.rollNo = String(row[rollIdx]).trim();
            }
            if (phaseIdx !== -1 && row[phaseIdx] !== undefined && row[phaseIdx] !== null && String(row[phaseIdx]).trim() !== "") {
              s.phase = String(row[phaseIdx]).trim();
            }
            if (batchIdx !== -1 && row[batchIdx] !== undefined && row[batchIdx] !== null && String(row[batchIdx]).trim() !== "") {
            ...
            // Validate student name is not some instructions, titles, or headers
            const isHeaderOrInstruction = 
              studentName.toLowerCase().includes("student name") || 
              studentName.toLowerCase().includes("evaluation ledger") ||
              studentName.toLowerCase().includes("secondary school") ||
              studentName.toLowerCase().includes("academic record") ||
              studentName.toLowerCase().includes("ledger overview") ||
              studentName.toLowerCase().includes("grading conversion") ||
              studentName.toLowerCase().includes("instructions") ||
              studentName.toLowerCase().includes("guidelines") ||
              studentName.length < 2 ||
              studentName === "—" || 
              studentName === "";

            if (!isHeaderOrInstruction) {
              // Student doesn't exist yet but has a valid name: let's AUTO-ENROLL them!
              const newId = studentId && studentId.startsWith("CS-") ? studentId : `CS-${Date.now().toString().slice(-4)}-${Math.floor(100 + Math.random() * 900)}`;
              const parsedRoll = rollIdx !== -1 && row[rollIdx] !== undefined && row[rollIdx] !== null && String(row[rollIdx]).trim() !== ""
                ? String(row[rollIdx]).trim() 
                : (updatedStudents.length + 1).toString();
              const parsedGrade = gradeIdx !== -1 && row[gradeIdx] !== undefined && row[gradeIdx] !== null && String(row[gradeIdx]).trim() !== ""
                ? String(row[gradeIdx]).trim()
                : (classFilter !== "all" && classFilter !== "" ? classFilter : "Class 3A");
              const parsedPhase = phaseIdx !== -1 && row[phaseIdx] !== undefined && row[phaseIdx] !== null && String(row[phaseIdx]).trim() !== ""
                ? String(row[phaseIdx]).trim()
                : (phaseFilter || "Phase 1");
              const parsedBatch = batchIdx !== -1 && row[batchIdx] !== undefined && row[batchIdx] !== null && String(row[batchIdx]).trim() !== ""
                ? String(row[batchIdx]).trim()
                : (batchFilter || "2083 BS");

              const customScores: ScoreComponents = {
                participation: partIdx !== -1 ? parseScore(row[partIdx], 10) : 8,
                homework: hwIdx !== -1 ? parseScore(row[hwIdx], 10) : 8,
                mcq: mcqIdx !== -1 ? parseScore(row[mcqIdx], 30) : 22,
                project: projIdx !== -1 ? parseScore(row[projIdx], 30) : 22,
                lab: labIdx !== -1 ? parseScore(row[labIdx], 20) : 14
              };
              ...
              const newRec: StudentRecord = {
                ...
                remarks: generateDefaultRemarks(customScores),
                strengths: generateDefaultStrengthsAndImprovements(customScores, studentName).strengths,
                areasOfImprovement: generateDefaultStrengthsAndImprovements(customScores, studentName).areasOfImprovement
              };

              updatedStudents.push(newRec);
              enrolledCount++;
              updatedCount++;
            }
          }
        ...
        if (updatedCount > 0) {
          setStudents(updatedStudents);
          setHasUnsaved(true); // Mark as unsaved to trigger secure cloud synchronization!
          ...
          let successMessage = `🎉 Success: Uploaded and saved updated evaluations for ${updatedCount} students!`;
          if (enrolledCount > 0) {
            successMessage = `🎉 Success: Synced marks for ${updatedCount - enrolledCount} students and automatically enrolled ${enrolledCount} new students!`;
          }
          triggerStatus(successMessage);
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans selection:bg-slate-200">
      ...
      {conflictModalOpen && conflicts.length > 0 && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto print:hidden animate-fade-in">
          <div 
            className="bg-white rounded-2xl max-w-5xl w-full overflow-hidden shadow-2xl border border-slate-200/65 my-8 flex flex-col max-h-[90vh] animate-scale-up"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-base tracking-tight flex items-center gap-2">
                    Database Merge Conflict Detected
                  </h3>
                  <p className="text-xs text-slate-400">
                    We detected <span className="font-bold text-amber-400">{conflicts.length}</span> student record(s) with conflicting edits. Choose which version to keep.
                  </p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 space-y-8">
              {conflicts.map((conflict, index) => {
                ...
                // Helper to see if values differ
                const dName = local.name !== cloud.name;
                const dRoll = (local.rollNo || "") !== (cloud.rollNo || "");
                const dGrade = local.grade !== cloud.grade;
                const dPhase = (local.phase || "") !== (cloud.phase || "");
                const dBatch = (local.batch || "") !== (cloud.batch || "");
                
                const dPart = local.scores.participation !== cloud.scores.participation;
                const dHw = local.scores.homework !== cloud.scores.homework;
                const dMcq = local.scores.mcq !== cloud.scores.mcq;
                const dProj = local.scores.project !== cloud.scores.project;
                const dLab = local.scores.lab !== cloud.scores.lab;

                const getScoreClass = (isDiff: boolean, isSelected: boolean) => {
                  if (isDiff) {
                    return isSelected 
                ...
                const isStrengthsDiff = local.strengths !== cloud.strengths;
                const isImprovementDiff = local.areasOfImprovement !== cloud.areasOfImprovement;

                return (
                  <div key={conflict.id} className="bg-white border border-slate-200/80 rounded-2xl shadow-xs overflow-hidden">
                    {/* Record Header */}
                    <div className="bg-slate-100 px-5 py-3 flex items-center justify-between border-b border-slate-200/65">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs text-slate-400 font-mono">({conflict.id})</span>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-5">
                      ...
                      <div 
                        onClick={() => {
                          ...
                          updated[index].choice = "local";
                          setConflicts(updated);
                        }}
                        className={`group relative border-2 rounded-xl p-4 transition-all cursor-pointer select-none flex flex-col gap-3.5 ${
                      ...
                      >
                        ...
                        {/* Metadata details */}
                        <div className="grid grid-cols-2 gap-2 bg-white/70 p-2.5 rounded-lg border border-slate-100 text-[11px] font-medium text-slate-600">
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Class</span>
                            <span className={dGrade ? "text-amber-800 font-bold bg-amber-100/50 px-1 rounded" : ""}>
                              {local.grade}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Roll Number</span>
                            <span className={dRoll ? "text-amber-800 font-bold bg-amber-100/50 px-1 rounded" : ""}>
                              {local.rollNo || "N/A"}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Academic Batch</span>
                            <span className={dBatch ? "text-amber-800 font-bold bg-amber-100/50 px-1 rounded" : ""}>
                              {local.batch || "N/A"}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Evaluation Phase</span>
                            <span className={dPhase ? "text-amber-800 font-bold bg-amber-100/50 px-1 rounded" : ""}>
                              {local.phase || "Phase 1"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Scores & Metrics</span>
                          <div className="grid grid-cols-1 gap-1.5 text-xs">
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dPart, conflict.choice === "local")}`}>
                              <span>Participation</span>
                              <span>{local.scores.participation}/10</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dHw, conflict.choice === "local")}`}>
                              <span>Homework</span>
                              <span>{local.scores.homework}/10</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dMcq, conflict.choice === "local")}`}>
                              <span>Assessment (MCQ)</span>
                              <span>{local.scores.mcq}/30</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dProj, conflict.choice === "local")}`}>
                              <span>Creative Projects</span>
                              <span>{local.scores.project}/30</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dLab, conflict.choice === "local")}`}>
                              <span>Practical Lab</span>
                              <span>{local.scores.lab}/20</span>
                            </div>
                          </div>
                        </div>

                        {/* Strengths & Improvement Previews */}
                        <div className="text-[11px] bg-slate-50/70 p-2.5 rounded-lg space-y-1.5 border border-slate-100 mt-1">
                          <div>
                            <strong className="text-slate-700 block">Strengths:</strong>
                            <p className={`line-clamp-2 text-slate-500 leading-tight ${isStrengthsDiff ? "bg-amber-50 text-amber-900 rounded px-1" : ""}`}>
                              {local.strengths || "No strengths defined."}
                            </p>
                          </div>
                          <div>
                            <strong className="text-slate-700 block">Areas of Improvement:</strong>
                            <p className={`line-clamp-2 text-slate-500 leading-tight ${isImprovementDiff ? "bg-amber-50 text-amber-900 rounded px-1" : ""}`}>
                              {local.areasOfImprovement || "No improvement areas defined."}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div 
                        onClick={() => {
                          const updated = [...conflicts];
                          updated[index].choice = "cloud";
                          setConflicts(updated);
                        }}
                        className={`group relative border-2 rounded-xl p-4 transition-all cursor-pointer select-none flex flex-col gap-3.5 ${
                          conflict.choice === "cloud" 
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2 bg-white/70 p-2.5 rounded-lg border border-slate-100 text-[11px] font-medium text-slate-600">
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Class</span>
                            <span className={dGrade ? "text-blue-800 font-bold bg-blue-100/50 px-1 rounded" : ""}>
                              {cloud.grade}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Roll Number</span>
                            <span className={dRoll ? "text-blue-800 font-bold bg-blue-100/50 px-1 rounded" : ""}>
                              {cloud.rollNo || "N/A"}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Academic Batch</span>
                            <span className={dBatch ? "text-blue-800 font-bold bg-blue-100/50 px-1 rounded" : ""}>
                              {cloud.batch || "N/A"}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">Evaluation Phase</span>
                            <span className={dPhase ? "text-blue-800 font-bold bg-blue-100/50 px-1 rounded" : ""}>
                              {cloud.phase || "Phase 1"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Scores & Metrics</span>
                          <div className="grid grid-cols-1 gap-1.5 text-xs">
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dPart, conflict.choice === "cloud")}`}>
                              <span>Participation</span>
                              <span>{cloud.scores.participation}/10</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dHw, conflict.choice === "cloud")}`}>
                              <span>Homework</span>
                              <span>{cloud.scores.homework}/10</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dMcq, conflict.choice === "cloud")}`}>
                              <span>Assessment (MCQ)</span>
                              <span>{cloud.scores.mcq}/30</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dProj, conflict.choice === "cloud")}`}>
                              <span>Creative Projects</span>
                              <span>{cloud.scores.project}/30</span>
                            </div>
                            <div className={`flex items-center justify-between px-2.5 py-1 rounded border ${getScoreClass(dLab, conflict.choice === "cloud")}`}>
                              <span>Practical Lab</span>
                              <span>{cloud.scores.lab}/20</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-[11px] bg-slate-50/70 p-2.5 rounded-lg space-y-1.5 border border-slate-100 mt-1">
                          <div>
                            <strong className="text-slate-700 block">Strengths:</strong>
                            <p className={`line-clamp-2 text-slate-500 leading-tight ${isStrengthsDiff ? "bg-blue-50 text-blue-950 rounded px-1" : ""}`}>
                              {cloud.strengths || "No strengths defined."}
                            </p>
                          </div>
                          <div>
                            <strong className="text-slate-700 block">Areas of Improvement:</strong>
                            <p className={`line-clamp-2 text-slate-500 leading-tight ${isImprovementDiff ? "bg-blue-50 text-blue-950 rounded px-1" : ""}`}>
                              {cloud.areasOfImprovement || "No improvement areas defined."}
                            </p>
                          </div>
                        </div>
                      </div>
            ...
            <div className="bg-slate-50 px-6 py-4 flex items-center justify-between border-t border-slate-200 shrink-0">
              <button
                ...
                onClick={() => {
                  ...
                  setConflicts([]);
                  setTempCloudPayload(null);
                  triggerStatus("❌ Pull cancelled. No changes were made.");
                }}
                className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 bg-white hover:bg-slate-100 border border-slate-200 hover:border-slate-300 transition-all cursor-pointer"
              >
                Cancel & Close
              </button>
              <div className="flex items-center gap-3">
                <span className="text-[11px] text-slate-500 font-semibold hidden sm:inline">
                  All {conflicts.length} conflict(s) will be merged.
                </span>
                <button
                  type="button"
                  onClick={handleResolveConflicts}
                  disabled={isSyncing}
                  className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 hover:shadow-lg transition-all cursor-pointer flex items-center gap-2 disabled:bg-slate-350 disabled:cursor-not-allowed"
                >
                  {isSyncing ? (
                    <>
                    ...
                    <>
                      ...
                      <span>Confirm, Merge & Sync Cloud</span>
                    </>
                  )}
                </button>
              </div>
      ...
      <header className="bg-slate-900 text-white px-6 py-4 flex flex-col md:flex-row justify-between items-center shadow-md border-b border-slate-850 gap-4 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5 font-medium flex flex-wrap items-center gap-x-2.5 gap-y-1">
              <span className="inline-flex items-center gap-1.5 text-slate-300">
                <span className="inline-block w-2 h-2 rounded-full bg-blue-400" />
                EduGrade Evaluation Hub
              </span>
              {isOffline ? (
                <span className="inline-flex items-center gap-1 bg-rose-500/20 text-rose-400 px-2 py-0.5 rounded-full border border-rose-500/30 text-[9px] font-extrabold tracking-wider uppercase animate-pulse">
      ...
      <div className="bg-slate-900/10 border-b border-slate-250 py-3 px-6 flex flex-col xl:flex-row justify-between items-center gap-3 print:hidden shadow-sm z-30 bg-white">
        ...
        {activeStudent ? (
          <div className="text-xs font-semibold text-slate-600 flex flex-wrap items-center gap-2">
            <span className="hidden sm:inline">Evaluating Record:</span>
            ...
            {/* Cloud Sync Status & Manual Push/Pull Group */}
            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200/80 p-1 rounded-xl shrink-0">
              <div 
                className="flex items-center gap-1.5 text-[11px] font-medium px-2 py-1 text-slate-600 cursor-help"
                title={lastSyncTime ? `Last database handshake occurred at ${lastSyncTime}` : "Connecting to master database..."}
              >
                <Cloud className={`h-3.5 w-3.5 ${hasUnsaved ? "text-amber-500 animate-pulse" : "text-blue-500"} ${isSyncing ? "animate-bounce" : ""}`} />
                <div className="hidden md:inline">
                  {isSyncing ? (
                    ...
                    <span className="text-amber-700 font-bold">Unsaved Cloud</span>
                  ) : (
                    <span className="text-emerald-700 font-bold">In Sync</span>
                  )}
                </div>
                {lastSyncTime && (
                  <span className="text-[10px] font-normal text-slate-400 hidden lg:inline">({lastSyncTime})</span>
                )}
              </div>
              ...
              <button
                ...
                title="Pull latest evaluations from cloud (Loads marks edited on other devices)"
              >
              ...
              <button
                ...
                title="Push current evaluations to cloud immediately"
              >
